...
import "../../../contracts/AMOs/implementations/convex/ConvexBPAMO.sol";
...
contract ConvexBPAMOTest is BaseTest {
  ...
  IConvexClaimZap public constant convexClaimZapAddress =
    IConvexClaimZap(0xDd49A93FDcae579AE50B4b9923325e9e335ec82B);
  ITreasury public constant treasury =
    ITreasury(0x8667DBEBf68B0BFa6Db54f550f41Be16c4067d60);
  ...
  IAgToken public agToken =
    IAgToken(0x1a7e4e63778B4f12a199C062f3eFdD288afCBce8);
  IERC20 public collateral = IERC20(0x1aBaEA1f7C830bD89Acc67eC4af516284b1bC33c);
  IMetaPool2 public mainPool =
    IMetaPool2(0xBa3436Fd341F2C8A928452Db3C5A3670d1d5Cc73);
  IConvexBaseRewardPool public baseReward =
  ...
  function setUp() public override {
    ...
    amoMinter = AMOMinter(
      deployUpgradeable(
        ...
        abi.encodeWithSelector(
          amoMinterImplementation.initialize.selector,
          coreBorrow
        )
      )
    );
    ...
    amo = ConvexAgEURvEUROCAMO(
      deployUpgradeable(
        ...
        abi.encodeWithSelector(
          ...
          IConvexBaseRewardPool(address(baseReward)),
          pidConvex
        )
      )
    );
    ...
    keeperJob = BPAMOJob(
      deployUpgradeable(
        ...
        abi.encodeWithSelector(
          keeperJobImplementation.initialize.selector,
          amoMinter,
          0
        )
      )
  ...
  function testPushRevertWrongLength(uint256 mintAmount, uint256 collatAmount)
    public
  {
  ...
  function testPushSimple(uint256 mintAmount) public {
    ...
    assertEq(
      agToken.balanceOf(address(mainPool)),
      prevAgTokenBalanceCurve + mintAmount
    );
    assertEq(
      collateral.balanceOf(address(mainPool)),
      prevCollateralBalanceCurve
    );
    ...
    assertEq(
      baseReward.balanceOf(address(amo)),
  ...
  function testPushGainDebtsMultiTx(
    ...
    assertEq(
      ...
      mint2ndAmount + netAgToken
    );
    if (netAgToken > mintAmount) {
      assertEq(amo.protocolDebts(IERC20(address(agToken))), 0);
      assertEq(
        amo.protocolGains(IERC20(address(agToken))),
        netAgToken - mintAmount
      );
    } else {
      assertEq(
        amo.protocolDebts(IERC20(address(agToken))),
        mintAmount - netAgToken
      );
      assertEq(amo.protocolGains(IERC20(address(agToken))), 0);
    }
  ...
  function testMultiPushGainDebtsMultiTx(
    ...
    uint256 prevDebts = 0;
    for (uint256 i = 0; i < txsAmount.length; i++) {
      if (idxAdd < 10 && idxSwap < 10)
        swapForAgEUR[i] = bound(swapForAgEUR[i], 0, 2);
      else if (idxAdd < 10) swapForAgEUR[i] = bound(swapForAgEUR[i], 1, 2);
      else swapForAgEUR[i] = 0;
      if (swapForAgEUR[i] == 0) {
        mintAmounts[idxAdd] = bound(
          mintAmounts[idxAdd],
          1 ether,
          5_000_000 ether
        );
  ...
  function testPullSimple(uint256 mintAmount, uint256 propWithdrawAmount)
    public
  {
    mintAmount = bound(mintAmount, 1 ether, 10_000_000 ether);
    ...
    assertGe(
      agToken.balanceOf(address(mainPool)),
      prevAgTokenBalanceCurve + mintAmount - propWithdrawAmount
    );
    assertEq(
      collateral.balanceOf(address(mainPool)),
      prevCollateralBalanceCurve
  ...
  function testMultiPullGainDebtsMultiTx(
    ...
    uint256 prevDebts = 0;
    for (uint256 i = 0; i < txsAmount.length; i++) {
      if (idxLiquidity < 10 && idxSwap < 10)
        swapForAgEUR[i] = bound(swapForAgEUR[i], 0, 3);
      else if (idxLiquidity < 10)
        swapForAgEUR[i] = bound(swapForAgEUR[i], 2, 3);
      else swapForAgEUR[i] = bound(swapForAgEUR[i], 0, 1);
      if (swapForAgEUR[i] == 0) {
        liquidityAmounts[idxLiquidity] = bound(
        ...
        assertEq(
          amo.lastBalances(IERC20(address(agToken))),
          liquidityAmounts[idxLiquidity] + netAgToken
        );
      ...
      if (swapForAgEUR[i] == 1) {
        ...
        liquidityAmounts[idxLiquidity] = idxLiquidity == 0
          ? 0
          : (liquidityAmounts[idxLiquidity] *
            mainPool.calc_withdraw_one_coin(
              baseReward.balanceOf(address(amo)),
              STABLE_IDX
            )) / 10**9;
        {
          uint256 maxDebtToReimburse = amoMinter.amoDebts(
            IAMO(address(amo)),
            IERC20(address(agToken))
          );
        ...
        assertEq(
          amo.lastBalances(IERC20(address(agToken))),
          netAgToken - liquidityAmounts[idxLiquidity]
        );
  ...
  function testClaimRewards(uint256 mintAmount, uint256 CRVRewards) public {
    mintAmount = bound(mintAmount, 1 ether, 10_000_000 ether);
    CRVRewards = bound(CRVRewards, 1 ether, 10_000_000 ether);

    deal(address(CRV), address(baseReward), CRVRewards);
    // fake a non null incentives program
    vm.prank(address(convexBooster));
    baseReward.queueNewRewards(CRVRewards);
    ...
    uint256 totSupply = baseReward.totalSupply();
    uint256 mySupply = baseReward.balanceOf(address(amo));

    vm.warp(block.timestamp + 7 * 3600 * 24);
    amo.claimRewards(new IERC20[](0));
    assertApproxEqAbs(
      CRV.balanceOf(address(amo)),
      (CRVRewards * mySupply) / totSupply,
      10**9
    );
    // an additional reward contract should be deployed to receive CVX rewards
    assertGt(CVX.balanceOf(address(amo)), 0);
  }

  // ================================== HELPERS ==================================

  function _getNavBPAMO() public view returns (uint256 nav) {
    uint256 amoLpTokenSupply = baseReward.balanceOf(address(amo));
