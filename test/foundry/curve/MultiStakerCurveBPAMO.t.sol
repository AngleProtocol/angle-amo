...
contract MultiStakerCurveBPAMOTest is BaseTest {
    ...
    function testSetStakeDAOProportionWrongCaller(uint256 propStakeDAO) public {
        propStakeDAO = bound(propStakeDAO, 0, 10**9);
        vm.startPrank(_alice);
        vm.expectRevert(BaseAMOStorage.NotApproved.selector);
        amo.setStakeDAOProportion(propStakeDAO);
        vm.stopPrank();
    }

    function testSetStakeDAOProportionTooLarge(uint256 propStakeDAO) public {
        propStakeDAO = bound(propStakeDAO, 10**9 + 1, type(uint256).max);
        vm.startPrank(_GOVERNOR);
        vm.expectRevert(BaseCurveAMO.IncompatibleValues.selector);
        amo.setStakeDAOProportion(propStakeDAO);
        vm.stopPrank();
    }

    function testSetStakeDAOProportionSuccess(uint256 propStakeDAO) public {
        propStakeDAO = bound(propStakeDAO, 0, 10**9);
        vm.startPrank(_GOVERNOR);
        amo.setStakeDAOProportion(propStakeDAO);
        ...
        vm.startPrank(address(keeperJob));
        amo.setStakeDAOProportion(propStakeDAO);
        vm.stopPrank();
    }
    ...
    function testPushSimple(uint256 mintAmount, uint256 propStakeDAO) public {
    ...
    function testMultiPushGainDebtsMultiTx(
        ...
        // 10 wei because the precision errors compounds
        assertApproxEqAbs(
            _liquidityGauge.balanceOf(address(amo)),
            ((mainPool.totalSupply() - prevlpTokenSupply) * propStakeDAO) / 10**9,
            10 wei
    ...
    function testPushRevertWithdrawalFee(
        ...
        vm.expectRevert(MultiStakerCurveBPAMO.WithdrawFeeTooLarge.selector);
    ...
    function testPullSimple(
        ...
        uint256 prevAgTokenBalanceCurve;
        uint256 prevCollateralBalanceCurve;
        uint256 prevlpTokenSupply;
        uint256 prevGovAgEURBalance;

        {
            ...
            prevAgTokenBalanceCurve = agToken.balanceOf(address(mainPool));
            prevCollateralBalanceCurve = collateral.balanceOf(address(mainPool));
            prevlpTokenSupply = mainPool.totalSupply();
            prevGovAgEURBalance = agToken.balanceOf(_GOVERNOR);
    ...
    function testMultiPullGainDebtsMultiTxBothStaker(
        uint256 propStakeDAO,
    ...
    // repliacte the previous test with null values because coverage doesn't detect it well
    function testMultiPullGainDebtsMultiTxOnlyConvex(
        uint256[5] memory liquidityAmounts,
        uint256[5] memory txsAmount,
        uint256[10] memory swapForAgEUR
    ) public {
        ...
        for (uint256 i = 0; i < txsAmount.length; i++) {
            ...
            assertEq(baseReward.balanceOf(address(amo)), (mainPool.totalSupply() - prevlpTokenSupply));
        }
    ...
    function testMultiPullGainDebtsMultiTxOnlyStakeDAO(
        uint256[5] memory liquidityAmounts,
        uint256[5] memory txsAmount,
        uint256[10] memory swapForAgEUR
    ) public {
        vm.startPrank(_GOVERNOR);
        amoMinter.addTokenRightToAMO(IAMO(address(amo)), IERC20(address(collateral)), type(uint256).max);
        amo.setStakeDAOProportion(10**9);
        vm.stopPrank();
    ...
    function testClaimRewards(
        ...
        CRVConvexRewards = bound(CRVConvexRewards, 1 ether, 10_000_000 ether);
        propStakeDAO = bound(propStakeDAO, 0, 10**9);
        vm.startPrank(_GOVERNOR);
        amo.setStakeDAOProportion(propStakeDAO);
        vm.stopPrank();

        deal(address(CRV), address(baseReward), CRVConvexRewards);
        ...
        baseReward.queueNewRewards(CRVConvexRewards);
        // fake a non null incentives program on Curve
        ...
        uint256 convexTotSupply = baseReward.totalSupply();
        uint256 convexMySupply = baseReward.balanceOf(address(amo));

        vm.warp(block.timestamp + 7 * 3600 * 24);
        amo.claimRewards(new IERC20[](0));
        assertApproxEqAbs(
            CRV.balanceOf(address(amo)),
            (CRVRewardAmount * mySupply) / totSupply + (CRVConvexRewards * convexMySupply) / convexTotSupply,
            10**9
        );
    ...
    function _getNavBPAMO() public view returns (uint256 nav) {
        ...
        amoLpTokenSupply += baseReward.balanceOf(address(amo));
